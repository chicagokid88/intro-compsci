...
def getWordScore(word, n):
    ...
    #    print("please enter a string for word")
    #assert type(n)==int
    #except AssertionError:
    ...
    #compute sum of all of the words
    totalScore=0
    for i in word:
        totalScore += SCRABBLE_LETTER_VALUES[i]
    
    totalScore = totalScore*lengthWord
...
def updateHand(hand, word):
    ...
    for i in word:
        copiedHand[i] = copiedHand.get(i,0) - 1
    return copiedHand
...
def isValidWord(word, hand, wordList):
    ...
    # check that the word is not the emptry string
    try:
    ...
    try:
        assert(word in wordlist is True)
    except AssertionError:
        return False
